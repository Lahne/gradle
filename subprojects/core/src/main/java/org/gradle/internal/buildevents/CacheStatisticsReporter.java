...
public class CacheStatisticsReporter implements TaskOutputCacheDiagnosticsListener {
    ...
    @Override
    public void buildFinished(TaskOutputCacheDiagnostics diagnostics) {
        StyledTextOutput textOutput = textOutputFactory.create(BuildResultLogger.class, LogLevel.LIFECYCLE);
        final int cacheableTasks = diagnostics.getCacheableTasks();
        final int executedTasks = diagnostics.getExecutedTasks();
        final int cachedTasks = diagnostics.getCachedTasks();
        textOutput.println();
